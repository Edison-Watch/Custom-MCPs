 ...
 * Many MCP clients serialize an unset optional string field as `""`; without
 * this, `cwd: ""` would slip past the `?? default` fallback and fail validation.
 *
 ...
 * the field REQUIRED. Wrapping the preprocess in `.optional()` keeps it optional
 * for callers while still normalizing a blank value to undefined.
 */
...
/** Keep only the last non-empty line of a chunk, for a compact progress message. */
function lastLine(chunk: string): string | undefined {
  const lines = chunk.split('\n').map((l) => l.trimEnd())
  for (let i = lines.length - 1; i >= 0; i--) {
    const line = lines[i]
    if (line && line.trim()) return line.trim().slice(0, 200)
  }
  return undefined
}
 ...
 * per-call `cwd` argument wins over both. Lets a marketplace entry pin the
 * working directory once instead of every caller passing it.
 */
  ...
  server.registerTool(
    'run',
    {
      ...
      inputSchema: {
        ...
        stdin: optionalNonBlank(z.string().optional()).describe(
          'Optional data piped to the command on stdin, followed by EOF.'
        ),
        timeout_ms: z
          .number()
          .int()
    ...
    async ({ command, cwd, stdin, timeout_ms }, extra) => {
      ...
      // Trim here (not in the shared optionalNonBlank helper, which also normalizes
      // `stdin`, where surrounding whitespace is meaningful data): a padded-but-valid
      // path like `'/repo '` from a templated config should resolve, not fail.
      ...
      // look hung. Emit a progress notification on each new output line plus a
...
// Only auto-start when run as the CLI entry point, not when imported by a test.
