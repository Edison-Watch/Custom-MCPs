/**
 * Pluggable auth for fleet servers - the shared TypeScript port.
 *
 ...
 * (see ./jwt.ts).
 *
 * This is the single source of truth for the fleet's auth contract: each
 * TypeScript connector re-exports it from its own `src/auth.ts`, so the
 * verify layer is defined once rather than copied per server. See
 * ../README.md and ../../docs/mcp_commodity_fleet_strategy.md (§6 Auth model).
 */
 ...
 *
 * The same fail-closed rule applies to `AUTH_MODE` itself: if it is *defined*
 * but not one of the known modes (including an empty or whitespace-only value),
 * we return it verbatim as an invalid mode so `checkAuth` rejects it, rather
 * than letting an empty string fall through to `open`. Only a truly unset
 * `AUTH_MODE` takes the token-based default.
 *
...
export function resolveAuthMode(env: AuthEnv): AuthMode | string {
  ...
  // AUTH_MODE was set to *something* that is not a known mode (an unknown value,
  // or empty/whitespace-only): surface it as an explicit reject in checkAuth.
  // `raw || "invalid-auth-mode"` keeps an empty string from ever meaning `open`.
  if (env.AUTH_MODE !== undefined) return raw || "invalid-auth-mode";
  // AUTH_MODE truly unset. Distinguish absent AUTH_TOKEN (undefined -> open)
  // from present-but-empty ("" -> bearer, which then 500s as misconfigured) so
  // an empty token never disables auth.
